...
static void main(String[] args) {
  ...
  String dbConnName = dbConnList[0].toString()

  Integer CurrentCust = 0
  Integer AllCusts = 0
  def StageCdList = ["MIN_REQ","BCS_CHECK", "PA_CALC", "CLD1", "CLD2", "RESTRUCT"]
  String stageCd = StageCdList[0].toString()
    ...
    frame(title: 'PA CALL Run', size: [200, 230], show: true, locationRelativeTo: null, defaultCloseOperation: EXIT_ON_CLOSE) {
      ...
      panel(border: compoundBorder([emptyBorder(10), titledBorder('Choose Stage')])) {
        comboBox(
                ...
                items:StageCdList,
                editable:true,
                actionPerformed:{event-> stageCd = event.source.selectedItem}
        )
        comboBox(
                id:'dbConn',
                items:dbConnList,
        ...
        button text: 'Run Call', actionPerformed: {
          ...
          try {
            ...
            custIds.each {c ->
               ...
               *******************************************************************************************************/
              openRccRootStatement(SQL_ENTITY_CUSTOMER, customerId, packId, stageCd)
              ...
              openRccRootStatement(SQL_ENTITY_SPR_EXT_CALL_ROUTE, customerId, packId, stageCd)

              /*******************************************************************************************************
               * Рекурсия по всем дочерним сущностям Customer - для открытия курсоров БД по ним (без выполнения запроса, только подготовка)
               *******************************************************************************************************/
              openChildInputStatements(customerId, packId, stageCd, packGroupBy, SQL_ENTITY_CUSTOMER, false)
